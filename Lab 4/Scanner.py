...
class Scanner:
    ...
    def scan(self, file_path: str):
        ...
        fa_identifier = FiniteAutomata("FA_identifiers.in")
        fa_numerical_constants = FiniteAutomata("FA_numerical_const.in")
        ...
        for token in tokens:
            if token in self.__operators:
            ...
            elif fa_identifier.accepts(token):
                ...
                self.__pif.add((token, self.__symbol_table.get_identifier_position(token)), "identifier")
            elif fa_numerical_constants.accepts(token):
                token = int(token)
